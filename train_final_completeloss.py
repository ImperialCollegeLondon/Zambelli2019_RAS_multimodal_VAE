...
logs_path = './logs/mvae_final_completeloss'
...
class VariationalAutoencoder(object):
    ...
    def _create_network(self):
        ...
        self.x_reconstr, self.x_log_sigma_sq = self._create_mod_variational_networ(['mod0_2','mod1_2','mod2_2','mod3_2','mod4_2'],'size_slices')
    ...
    def _create_loss_optimizer(self):
        with tf.name_scope('Loss_Opt'):
                        ...
                        self.reconstr_loss = tf.reduce_mean(self.tmp_costs[0]+ self.tmp_costs[1] + self.tmp_costs[2] + self.tmp_costs[3] + self.tmp_costs[4])
...
def train_whole(sess,vae, input_data, learning_rate=0.0001, batch_size=100, training_epochs=10, display_step=1, vae_mode=True, vae_mode_modalities=True):
    ...
    save_path = vae.saver.save(vae.sess, "./models/final_completeloss_all_conf_"+param_id+".ckpt")
        ...
        if(param_id==1):
            ...
            network_architecture = \
                                   {'n_input':28,\
                                    ...
                                    'size_slices':[8, 8, 2, 2, 8],\
                                        # BECAREFUL The number of slice should be equal to the number of _mod_ network
                                    'size_slices_shared':[10, 10, 3, 3, 10],\
                                        # BECAREFUL The sum of the dimensions of the slices, should be equal to the last dec_shared
                                    'mod0':[20,10],\
                                    'mod1':[20,10],\
                                    'mod2':[5,3],\
            ...
            network_architecture = \
                               {'n_input':28,\
                                    ...
                                    'size_slices':[8, 8, 2, 2, 8],\
                                        # BECAREFUL The number of slice should be equal to the number of _mod_ network
                                    'size_slices_shared':[40, 40, 10, 10, 40],\
                                        # BECAREFUL The sum of the dimensions of the slices, should be equal to the last dec_shared
                                    'mod0':[40],\
                                    'mod1':[40],\
                                    'mod2':[10],\
