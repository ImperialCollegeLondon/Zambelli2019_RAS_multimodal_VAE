...
with tf.Graph().as_default() as g:
    ...
    with tf.Session() as sess:
        ...
        # Test Maxime kinect data
        print('Test Maxime')
        sample_init = 0

        x_sample_nv_2_0 = np.append(X_test_Max_5_rescaled[sample_init+1:sample_init+batch_size+1,0:1],X_test_Max_5_rescaled[sample_init+1:sample_init+batch_size+1,2:3], axis=1)
        x_sample_nv_2_0 = np.append(x_sample_nv_2_0,x_sample_nv_2_0, axis=1)
        x_sample_nv_2_2 = np.append(X_test_Max_5_rescaled[sample_init:sample_init+batch_size,0:1],X_test_Max_5_rescaled[sample_init:sample_init+batch_size,2:3], axis=1)
        x_sample_nv_2_2 = np.append(x_sample_nv_2_2,x_sample_nv_2_2, axis=1)
        x_sample_nv_2 = np.append(x_sample_nv_2_0,x_sample_nv_2_2,axis=1)
        x_sample_nv_1 = np.full((x_sample_nv_2.shape[0],8),-2)
        x_sample_nv_3 = np.full((x_sample_nv_2.shape[0],12),-2)  
        x_sample_nv = np.append( x_sample_nv_1, np.append( x_sample_nv_2, x_sample_nv_3, axis=1), axis=1)

        x_reconstruct, x_reconstruct_log_sigma_sq = model.reconstruct(sess,x_sample_nv) 
        scipy.io.savemat("results/mvae_final_completeloss_testMaxime5.mat",{"x_reconstruct":x_reconstruct,"x_reconstruct_log_sigma_sq":x_reconstruct_log_sigma_sq,"x_sample":x_sample_nv})

        x_sample_nv_1 = np.full((x_sample_nv.shape[0],4),-2)  
        x_sample_nv_2 = x_reconstruct[:,0:4] 
        x_sample_nv_3 = np.full((x_sample_nv.shape[0],4),-2)  
        x_sample_nv_4 = x_reconstruct[:,8:12]       
        x_sample_nv_5 = np.full((x_sample_nv.shape[0],1),-2)  
        x_sample_nv_6 = x_reconstruct[:,16:17] 
        x_sample_nv_7 = np.full((x_sample_nv.shape[0],1),-2)  
        x_sample_nv_8 = x_reconstruct[:,18:19]
        x_sample_nv_9 = np.full((x_sample_nv.shape[0],4),-2)
        x_sample_nv_10 = x_reconstruct[:,20:24]
        x_sample_nv = np.append(x_sample_nv_1,
                                        np.append(x_sample_nv_2,
                                                  np.append(x_sample_nv_3,
        ...
        x_pred, x_pred_log_sigma_sq = model.reconstruct(sess,x_sample_nv) 
        scipy.io.savemat("results/mvae_final_completeloss_testPred_Maxime5.mat",{"x_pred":x_pred,"x_pred_log_sigma_sq":x_pred_log_sigma_sq,"x_sample":x_sample_nv})
...
with tf.Graph().as_default() as g:
    ...
    with tf.Session() as sess:
        ...
        x_sample_nv_2_0 = np.append(X_test_MartinaPiano_aligned2D[sample_init+1:sample_init+batch_size+1,0:1],X_test_MartinaPiano_aligned2D[sample_init+1:sample_init+batch_size+1,1:2], axis=1)
        x_sample_nv_2_0 = np.append(x_sample_nv_2_0,x_sample_nv_2_0, axis=1)
        x_sample_nv_2_2 = np.append(X_test_MartinaPiano_aligned2D[sample_init:sample_init+batch_size,0:1],X_test_MartinaPiano_aligned2D[sample_init:sample_init+batch_size,1:2], axis=1)
        x_sample_nv_2_2 = np.append(x_sample_nv_2_2,x_sample_nv_2_2, axis=1)
        x_sample_nv_2 = np.append(x_sample_nv_2_0,x_sample_nv_2_2,axis=1)
        x_sample_nv_1 = np.full((x_sample_nv_2.shape[0],8),-2)
        x_sample_nv_3 = np.full((x_sample_nv_2.shape[0],12),-2)  
        ...
        x_reconstruct, x_reconstruct_log_sigma_sq = model.reconstruct(sess,x_sample_nv) 
        scipy.io.savemat("results/mvae_final_completeloss_testMartinaPiano.mat",{"x_reconstruct":x_reconstruct,"x_reconstruct_log_sigma_sq":x_reconstruct_log_sigma_sq,"x_sample":x_sample_nv})
        ...
        x_pred, x_pred_log_sigma_sq = model.reconstruct(sess,x_sample_nv) 
        scipy.io.savemat("results/mvae_final_completeloss_testPred_MartinaPiano.mat",{"x_pred":x_pred,"x_pred_log_sigma_sq":x_pred_log_sigma_sq,"x_sample":x_sample_nv})
